...
UserSchema.methods.hashPassword = function (password) {
	if (this.salt && password) {
		return crypto.pbkdf2Sync(password, new Buffer(this.salt, 'base64'), 10000, 64, 'SHA1')
			.toString('base64');
	} else {
		return password;
	}
...
UserSchema.methods.hashVerificationCode = function (code) {
	if (this.salt && code) {
		...
		return crypto.pbkdf2Sync(code.toString(), new Buffer(this.salt, 'base64'), 10000, 64, 'SHA1')
			.toString('base64');
	} else {
